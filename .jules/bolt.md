...
## 2024-12-07 - [Optimize DB queries for Admin Dashboard]
**Learning:** In `src/routes/admin.js`, the `GET /api/v1/admin/stats` route executed 6 separate sequential queries to retrieve summary analytics for the dashboard (total users, active users, total sessions, connected sessions, credits deducted, and credits purchased). Each of these simple aggregations required its own SQLite statement compilation and a JS/C++ boundary crossing.
**Action:** Replaced the 6 sequential DB calls with a single optimized SQLite query leveraging subselects (e.g., `SELECT (SELECT COUNT(*) FROM users) as totalUsers, ...`). Bundling multiple independent global aggregations into a single query significantly reduces I/O round trips and execution time.
