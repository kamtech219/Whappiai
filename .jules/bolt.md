...
## 2024-12-07 - [Optimize DB queries for Admin Stats Dashboard]
**Learning:** In \`src/routes/admin.js\`, the \`/stats\` endpoint executed 6 separate sequential queries to retrieve summary analytics for the dashboard (total users, active users, total sessions, connected sessions, total credits deducted, total credits purchased). Grouping these independent global statistics into a single query using subselects reduces sequential I/O overhead and compilation time.
**Action:** Replaced sequential DB calls with a single optimized SQLite query using subselects (e.g., \`SELECT (SELECT COUNT(*) FROM users) as totalUsers, ...\`).
