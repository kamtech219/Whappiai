...
## 2026-04-26 - [Optimize DB Queries for Global Platform Stats Dashboard]
**Learning:** In `src/routes/admin.js`, retrieving multiple independent global scalar values (e.g., total users, active users, total sessions, total credits) using separate sequential SQLite queries causes unnecessary compilation and database round-trips.
**Action:** When gathering diverse, independent global statistics from SQLite, bundle multiple aggregations (like COUNT or SUM) into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM tableA) as totalA, (SELECT COUNT(*) FROM tableB) as totalB`). This significantly reduces sequential I/O overhead and compilation time.
