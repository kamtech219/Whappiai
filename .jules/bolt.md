...
## 2026-07-05 - [Optimize Global Aggregates in SQLite]
**Learning:** Performing multiple simple aggregate queries (like COUNT or SUM) sequentially on global application data incurs unnecessary JS/C++ boundary-crossing overhead in better-sqlite3.
**Action:** When gathering diverse, independent global statistics from SQLite, bundle multiple aggregations into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM tableA), (SELECT COUNT(*) FROM tableB)`) to significantly reduce compilation time and execution overhead.
