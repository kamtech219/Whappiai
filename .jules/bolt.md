...
## 2024-12-07 - [Optimize AI Content Safety Check]
**Learning:** In `src/services/ai.js`, the `isContentSafe` method instantiated an array of forbidden words and executed a `.toLowerCase()` conversion on the incoming string during every AI response. For high-volume or long-content generation, repetitive memory allocation and loop overhead degraded performance and increased garbage collection pressure.
**Action:** Lift static array definitions out of tight loops or frequently called methods. When performing case-insensitive substring matches against a static list of keywords, use a pre-compiled regular expression (`new RegExp(..., 'i')`) initialized at the module scope to avoid redundant array creation and string manipulation overhead.
