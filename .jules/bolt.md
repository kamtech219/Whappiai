...
## 2024-12-07 - [Optimize Database Queries in Admin Dashboard Route]
**Learning:** In `src/routes/admin.js`, multiple independent aggregate queries (e.g., `COUNT`, `SUM`) were executed sequentially to gather dashboard statistics. This causes unnecessary overhead from sequential I/O round-trips and JS/C++ boundary-crossing.
**Action:** Replaced sequential SQLite queries with a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM tableA), (SELECT SUM(...) FROM tableB)`), to significantly improve performance by bundling the execution. Also used `COALESCE(SUM(...), 0)` to handle aggregations gracefully when tables are empty.
