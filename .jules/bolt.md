...
## 2024-12-07 - [Optimize API Stats Dashboard Admin Endpoint]
**Learning:** In `src/routes/admin.js`, multiple independent scalar queries (counts and sums) for the `/api/v1/admin/stats` endpoint were executed sequentially, causing unnecessary JS/C++ boundary crossing and I/O overhead.
**Action:** When gathering diverse, independent global statistics from SQLite, bundle multiple aggregations (like COUNT or SUM) into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM tableA), (SELECT COUNT(*) FROM tableB)`) to significantly reduce overhead.
