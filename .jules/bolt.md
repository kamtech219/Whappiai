...
## 2025-04-28 - [Optimize Global DB Stats Query]
**Learning:** In `src/routes/admin.js`, multiple independent aggregations (like COUNT or SUM) across different tables were executed sequentially. This results in unnecessary sequential I/O overhead and compilation time in SQLite.
**Action:** When gathering diverse, independent global statistics from SQLite, bundle multiple aggregations into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM tableA) as countA, (SELECT COUNT(*) FROM tableB) as countB`). This significantly reduces sequential I/O roundtrips and execution time.
