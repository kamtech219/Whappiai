...
## 2024-05-24 - [Optimize ActivityLog insertions]
**Learning:** In `src/models/ActivityLog.js`, the `log()` method compiles an `INSERT` statement via `db.prepare(...)` on every single invocation. Since logging is a high-frequency operation called across the entire application (message sends, session creation, logins), repeatedly compiling the same SQLite statement adds unnecessary overhead and crosses the JS/C++ boundary redundantly.
**Action:** When working with `better-sqlite3`, cache prepared statements for frequently executed queries at the module or class level to avoid compilation overhead.
