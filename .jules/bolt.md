...
## 2024-12-07 - [Optimize Admin API stats dashboard database queries]
**Learning:** When fetching multiple diverse, independent metrics (e.g., total active users, connected sessions, total credits spent) for admin dashboards, performing 6 sequential `better-sqlite3` statements introduces considerable JS/C++ boundary-crossing latency, even if the individual statements are fast.
**Action:** Bundle multiple scalar aggregations into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM a), (SELECT COUNT(*) FROM b)`). This gathers all the disjoint global statistics in a single I/O round trip, drastically reducing compilation and boundary-crossing overhead. Always use `COALESCE` for `SUM` to handle empty tables gracefully.
