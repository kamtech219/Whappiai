...
## 2024-05-23 - [Optimize Admin Global Stats Query]
**Learning:** When gathering diverse, independent global statistics from SQLite (e.g., using better-sqlite3), executing multiple independent queries using `db.prepare(...).get()` incurs overhead for each JS/C++ boundary crossing and compilation time.
**Action:** Bundle multiple aggregations (like COUNT or SUM) into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM tableA), (SELECT COUNT(*) FROM tableB)`) to significantly reduce sequential I/O and boundary-crossing overhead. When translating JavaScript fallbacks (e.g., `|| 0`) into SQLite aggregations, wrap `SUM()` in `COALESCE(SUM(...), 0)` to safely handle cases where the table is empty and `SUM` returns `NULL`.
