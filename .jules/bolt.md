...
## 2026-06-12 - [Optimize independent aggregate queries with better-sqlite3]
**Learning:** Using multiple sequential `db.prepare(...).get()` calls for independent aggregations (like COUNT or SUM across different tables or conditions) introduces significant JS-to-C++ context switching and compilation overhead in `better-sqlite3`.
**Action:** When gathering diverse, independent global statistics from SQLite, bundle multiple aggregations into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM tableA), (SELECT COALESCE(SUM(amount), 0) FROM tableB)`) to minimize sequential I/O and boundary-crossing overhead.
