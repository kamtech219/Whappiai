...
## 2023-10-25 - [Optimize diverse SQLite statistics aggregations]
**Learning:** In the `better-sqlite3` environment of this backend, performing multiple independent, scalar aggregations (`SELECT COUNT(*)`, `SELECT SUM(*)`) consecutively incurs repetitive compilation and sequential I/O execution overhead.
**Action:** Bundle these multiple statistics into a single query using subselects (`SELECT (SELECT COUNT(*) FROM a), (SELECT SUM(*) FROM b)`) so the database engine resolves them efficiently in a single step, reducing synchronous latency.
