...
## 2024-12-07 - [Optimize DB queries for Admin API Stats]
**Learning:** In `src/routes/admin.js`, the `/stats` endpoint executed 6 separate sequential queries using `db.prepare` to retrieve independent platform-wide aggregations (e.g., total users, active users, total sessions, credits). While better-sqlite3 is fast, crossing the JavaScript/C++ boundary and performing multiple table scans sequentially adds up, especially since these aggregations are independent.
**Action:** Bundling independent aggregations (like COUNT or SUM) into a single query using subselects (e.g., `SELECT (SELECT COUNT(*) FROM users) as totalUsers, ...`) significantly reduces sequential I/O and boundary-crossing overhead, resulting in an ~45% speedup for this endpoint.
