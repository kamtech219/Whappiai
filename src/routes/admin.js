...
router.get('/stats', requireAdmin, asyncHandler(async (req, res) => {
    ...
    // ⚡ Bolt: Single optimized query for all scalar admin stats using subselects.
    // This replaces 6 sequential database round-trips with a single query execution,
    // significantly reducing SQLite compilation and I/O overhead on the dashboard load.
    ...
    const totalUsers = statsQuery.totalUsers || 0;
    const activeUsers = statsQuery.activeUsers || 0;
    const totalSessions = statsQuery.totalSessions || 0;
    const connectedSessions = statsQuery.connectedSessions || 0;
    const totalCreditsDeducted = statsQuery.totalCreditsDeducted || 0;
    const totalCreditsPurchased = statsQuery.totalCreditsPurchased || 0;
