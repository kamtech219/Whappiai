...
router.get('/stats', requireAdmin, asyncHandler(async (req, res) => {
    ...
    // ⚡ Bolt: Bundled multiple independent aggregation queries into a single SQLite query using subselects.
    // This reduces the number of C++/JS boundary crossings and overall query compilation/execution overhead.
    const aggregatedStats = db.prepare(`
        SELECT
            ...
            COALESCE((SELECT SUM(amount) FROM credit_history WHERE type = 'debit'), 0) as totalCreditsDeducted,
            COALESCE((SELECT SUM(amount) FROM credit_history WHERE type = 'purchase'), 0) as totalCreditsPurchased
    `).get();

    // 2. User Statistics
    const totalUsers = aggregatedStats.totalUsers;
    const activeUsers = aggregatedStats.activeUsers;

    // 3. Session Statistics
    const totalSessions = aggregatedStats.totalSessions;
    const connectedSessions = aggregatedStats.connectedSessions;

    // 4. Financial/Credit Statistics
    const totalCreditsDeducted = aggregatedStats.totalCreditsDeducted;
    const totalCreditsPurchased = aggregatedStats.totalCreditsPurchased;
