...
router.get('/stats', requireAdmin, asyncHandler(async (req, res) => {
    ...
    // ⚡ Bolt: Bundled multiple independent COUNT/SUM SQLite aggregations into a single query via subselects
    // to significantly reduce sequential I/O and JS-to-C++ context switching overhead.
    ...
    // 2. User Statistics
    const totalUsers = statsRow.totalUsers;
    const activeUsers = statsRow.activeUsers;

    // 3. Session Statistics
    const totalSessions = statsRow.totalSessions;
    const connectedSessions = statsRow.connectedSessions;

    // 4. Financial/Credit Statistics
    const totalCreditsDeducted = statsRow.totalCreditsDeducted;
    const totalCreditsPurchased = statsRow.totalCreditsPurchased;
